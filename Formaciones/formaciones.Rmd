...
# # local
local_form <- t1 %>% dplyr::count(formLocal) %>% mutate(S = "L")
names(local_form)[1] <- "form"
# # away
away_form <- t1 %>% dplyr::count(formVisita) %>% mutate(S = "V")
...
Premier League
```{r c5,cache=T}
pform3 <- ggplot(t2[which(t2$league=="Premier League"),], aes(x = form)) + 
...
BBVA
```{r c6,cache=T}
pform4 <- ggplot(t2[which(t2$league=="La Liga"),], aes(x = form)) + 
...
Serie A
```{r c7,cache=T}
pform5 <- ggplot(t2[which(t2$league=="Serie A"),], aes(x = form)) + 
...
Bundesliga
```{r c8,cache=T}
pform6 <- ggplot(t2[which(t2$league=="Bundesliga"),], aes(x = form)) + 
